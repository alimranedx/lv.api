...
class Arr
{
    ...
    public static function fill($size, $value, $starting_index=0) : array
    {
        return array_fill($starting_index, $size, $value);
    }

    public static function objToArray(object $object) : array
    {
        return get_object_vars($object);
    }
    public static function unsetElementByKeyAndValuesFromTwoDimensionalArray(array $array, array $remove_array) : array
    {
        foreach ($array as $key => &$arr) {
            foreach ($remove_array as $removalCriteria) {
                $match = true;
                foreach ($removalCriteria as $removalKey => $removalValue) {
                    if (!isset($arr[$removalKey]) || $arr[$removalKey] != $removalValue) {
                        $match = false;
                        break;
                    }
                }
        ...
        return self::filter($array,true);
    }
	
	public static function varExport($arr)
	{
		return var_export($arr, true);
	}

    public static function findChanges(array $oldArray, array $newArray, bool $showUnmatchedKeys = true): array
    {
        $allKeys = self::unique(self::merge(self::keys($oldArray), self::keys($newArray)));

        $changes = [];

        foreach ($allKeys as $key) {
            $existInOldArray = self::keyExists($key, $oldArray);
            $existInNewArray = self::keyExists($key, $newArray);
            if ($existInOldArray && $existInNewArray) {
            ...
            } elseif ($existInOldArray && $showUnmatchedKeys) {
                $changes[$key] = [
                    'old' => $oldArray[$key],
                    'new' => null,
                ];
            } elseif ($existInNewArray && $showUnmatchedKeys) {
                $changes[$key] = [
                    'old' => null,
                    'new' => $newArray[$key],
                ];
